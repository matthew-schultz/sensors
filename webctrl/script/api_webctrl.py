...
def insert_readings_into_database(conn, readings, sensor):
    ...
    for reading in readings:
        ...
        if reading_time.subtract(hours=10) > pendulum.instance(sensor.last_updated_datetime):
            reading_row = orm_webctrl.Reading(purpose_id=sensor.purpose_id, datetime=reading_time, value=reading_value, units=sensor.unit)
...
if __name__ == '__main__':
    ...
    conn = get_db_handler()
    sensors = conn.query(orm_webctrl.SensorInfo.purpose_id, orm_webctrl.SensorInfo.query_string, orm_webctrl.SensorInfo.last_updated_datetime, orm_webctrl.SensorInfo.unit).filter_by(script_folder=orm_webctrl.ScriptFolder.webctrl, is_active=True)
