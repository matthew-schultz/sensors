...
def get_csv_from_folder_not_in_db(conn, csv_filename):
    ...
    for data_sensor_info_mapping in csv_readings.columns[1:]:
        purpose_id, last_updated_datetime, unit = conn.query(orm_hobo.SensorInfo.purpose_id, orm_hobo.SensorInfo.last_updated_datetime, orm_hobo.SensorInfo.unit).filter_by(sensor_id=sensor_id, data_sensor_info_mapping=data_sensor_info_mapping, is_active=True).first()[:3]
