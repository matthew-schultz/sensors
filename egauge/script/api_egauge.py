            ...
            for row in readings.itertuples():
                ...
                for i, column_reading in enumerate(row[2:]):
                    ...
                    if purpose_sensor.data_sensor_info_mapping == column_name:
                        reading_row = orm_egauge.Reading(purpose_id=purpose_sensor.purpose_id, datetime=row_datetime, value=row_reading, units=purpose_sensor.unit)
...
if __name__ == '__main__':
    ...
    query_strings = [query_string[0] for query_string in conn.query(orm_egauge.SensorInfo.query_string).filter_by(script_folder=orm_egauge.ScriptFolder.egauge, is_active=True).distinct()]
