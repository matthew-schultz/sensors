"""
This script uses a timestamp log to keep track of and make egauge api requests for egauge sensor data.
...
def pull_egauge_data(sensor_id='31871', unit_of_time='m', output_file='./output.log', timestamp_log='./timestamp.log'):
    ...
    """
    try:
        current_timestamp = int(arrow.now().floor('second').timestamp)
        ...
        if not latest_timestamp_read_from_file:
            ...
            if api_start_timestamp > api_end_timestamp:
                raise ValueError('Error: api_start_timestamp ' + str(arrow.get(api_start_timestamp)) + ' was greater than api_end_timestamp '  + str(arrow.get(api_end_timestamp)))
            output_csv='c'
            delta_compression='C'
